...
Sea $(\Omega, \mathcal{A})$ un espacio medible cualquera fijo. Las siguientes son directrices de notación y definiciones iniciales:
 ...
 - El conjunto $\textbf{S}/\equiv$ se nota por $\mathbb{S}$
 - Un conjunto de medidas de probabilidad sobre $(\Omega, \mathcal{A})$ se denomina familia y se nota $\mathbb{P}$. Si es posible indexarlo mediante un espacio de dimensión finita se denomina familia paramétrica, caso contrario se denomina familia no paramétrica.
 - El conjunto que indexa una familia se denomina espacio de parámetros, para una familia paramétrica el espacio de parámetros tiene dimensión finita, para una familia no paramétrica, necesariamente infinita.
 - Dado $n$ un número natural, $1:n$ es el intervalo de los naturales entre 1 y $n$ inclusive.
 ...
 - Una función $s$ de $(1:n,\wp_n, p)$ en el espacio medible, $(\Omega, \mathcal{A})$ se denomina nube de puntos. Evidentemente toda nube de puntos es función simple y en consecuencia es medible.^[Revisar en Shao]
 - Dada una nube de puntos $s$, siendo función, tiene dominio $Dom(s)$, rango $Ran(s)$, y en virtud de ser medible tiene una medida asociada $\lambda_s$ sobre $(\Omega, \mathcal{A})$.
 ...
 - La relación $\equiv$ se define por por $s_1 \equiv s_2 : \Leftrightarrow \lambda_{s_1} = \lambda_{s_2}$. Evidentemente es una relación de equivalencia sobre $\textbf{S}$.
...
Sea $(\Omega, \mathcal{A})$ un espacio medible cualquera fijo y $s$ una nube de puntos cualquiera sobre $(\Omega, \mathcal{A})$. Los siguientes enunciados se tienen en consecuencia de las definiciones anteriores.
 ...
 - Toda familia se puede indexar a sí misma, en este sentido una medida de probabilidad puede ser un parámetro de si misma.

## Cielo aleatorio, cielo de puntos

Teorema y definición

Sean $(\Omega, \mathcal{A}, \lambda)$, con $\lambda$ en una familia $\mathbb{P}$, un espacio de medida cualquera fijo y $\mathbb{S}$ según lo mostrado previamente. La función de conjuntos $\lambda^*$ definida como

\begin{align}
\lambda^* &:  \wp(\mathbb{S}) \longrightarrow [0,1]\\
& S \subseteq \mathbb{S} \mapsto \lambda^*(S) := \lambda\left(\bigcup\limits_{s \in S} Ran(s) \right)
\end{align}

Es una función de medida. En particular el espacio de medida $(\mathbb{S}, \mathcal{P}(\mathbb{S}), \lambda^*)$ se denomina el cielo aleatorio de $(\Omega, \mathcal{A}, \lambda)$.

## Teorema de grandes números^[Confirmar]

Sean $(\mathbb{R}^k, \mathcal{B}^k, \lambda)$ un espacio de medida cualquera fijo y $(\mathbb{S}, \wp(\mathbb{S}), \lambda^*)$ su cielo de puntos. Sea $e$ el valor esperado visto como opreador sobre el cielo de puntos.

\begin{align}
e &:  (\mathbb{S}, \wp(\mathbb{S}), \lambda^*) \longrightarrow (\mathbb{R}^k, \mathcal{B}^k, \lambda)\\
& s \in \mathbb{S} \mapsto e(s) := E(s)
\end{align}

Con $E(s) = \sum_{i=1}^{n_s} p(\{i\})s(i)$.

Entonces

 - $e$ está bien definido.
 - $e$ es una variable aleatoria.
 - Su valor esperado es $\int \mathbb{1} d\lambda$ siendo $\mathbb{1}$ la función identidad.
