...
$\textbf{S}$ es un conjunto muy áspero. Es posible realizar algunas restricciones interesantes.
 ...
 - Sea una medida $\mu$ sobre el espacio medible $(\mathbb{N}, \wp(\mathbb{N}))$, sea $p_n$ la medida de probabilidad generada por $\mu$ sobre $(1:n, \wp_n)$. $\textbf{S} \mid_\mu$ es el subconjunto de $\textbf{S}$ cuyos elementos $s$ tienen como dominio $(1:n, \wp_n,p _n)$.
...
Teorema y definición^[No sirve]

Sean $(\Omega, \mathcal{A}, P, \lambda)$, con $P$ en una familia $\mathbb{P}_\lambda$, un espacio de medida cualquera fijo y $\mathbb{S}$ según lo mostrado previamente. La función de conjuntos $\lambda^*$ definida como
