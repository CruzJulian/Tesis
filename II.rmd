...
Sea $\Omega = \mathbb{R}^+\times\mathbb{R}^+\times\mathbb{R}^+\times\mathbb{R}^+\times \{ `r paste(unique(iris$Species), collapse = ", ")` \}$ y $\mathcal{A} = \mathcal{B}\times\mathcal{B}\times\mathcal{B}\times\mathcal{B}\times\wp$. Iris^[citar] es una nube de puntos $s$ de $(1:150, \wp, \dot{p})$ en $(\Omega, \mathcal{A})$.
...
$\textbf{S}$ es un conjunto muy áspero. Es posible realizar algunas restricciones interesantes.
 ...
 - En el espacio de medida $(\mathbb{N}, \wp, \mu)$, dado un natural $n$, existe una medida de probabilidad $p$ generada por $\mu$ sobre $(1:n, \wp)$, es decir: $p(A) = \frac{\mu(A)}{\mu(1:n)}$ así $\textbf{S} \mid_\mu$ es el subconjunto de $\textbf{S}$ cuyos elementos $s$ tienen como dominio $(1:n, \wp, p_n)$. En particular las restriciones finitas de $\ddot{\mu}$ se notan por $\ddot{p}$. Es evidente que $\ddot{p}$ es una medida de probabilidad uniforme.
...
En particular el espacio de medida $(\textbf{S} \mid_{\mu}, \sigma(P^*), P^*)$ se denomina un cielo aleatorio de $(\Omega, \mathcal{A}, P)$. Evidentemente $P^*(\textbf{S} \mid_{\mu,n}) = \mu(\{n\})$
...
tiene las propiedades:
 ...
 - Su valor esperado $E(e) = \int e dP^*$ es igual a $E(f) = \int f dP$ ???.
