...
Todo el fundamento teórico se ha escrito usando la integral de Bochner^[citar]. Al respecto es importante notar que, salvo se especifique lo contrario, $\Omega$ es un espacio de Banach. Es decir, un espacio vectorial normado y completo (citar, Cálculo avanzado José francisco caicedo def 1.68). De esta forma los resultados mostrados son válidos tanto para el caso univariado como el multivariado.

Es posible ver que el valor esperado y la varianza muestrales $\dddot{E}$ y $\dddot{Var}$ son funciones de $\textbf{S}$ en $\Omega$. El valor esperado muestral de $s$ es $\dddot{E}(s) = \sum_{i=1}^{n_s} p(\{i\})s(i)$. Que en algunos ámbitos se denomina media muestral. Asímismo, la varianza de $s$ es $Var(s) = \sum_{i=1}^{n_s} p(\{i\})(s(i) - E(s))^2$. Que en algunos ámbitos se denomina varianza poblacional.^[Evidentemente los nombres tradicionales de estas cantidades no coinciden con los aquí puestos. No se volverán a mencionar para no dar lugar a confusiones.]
...
Sea $\Omega = \mathbb{R}\times\mathbb{R}\times\mathbb{R}\times\mathbb{R}\times \{ `r paste(unique(iris$Species), collapse = ", ")` \}$ y $\mathcal{A} = \mathcal{B}\times\mathcal{B}\times\mathcal{B}\times\mathcal{B}\times\wp$. Iris^[@citar iris] es una nube de puntos de $(1:150, \wp, \dot{p})$ en $(\Omega, \mathcal{A})$.
...
Dada una función medible $f$ de $(\Omega, \mathcal{B}, P)$ en $\mathbb{R}$, a raíz de la definición de $P^n$ se tiene que:
...
Para un evento cualquiera $A \in \mathcal{B}$ se tiene por definición que $P_s(A) = \int \chi_A dp$, que es igual a $\dddot{E}|_{n_s}(\chi_A)$, cuyo valor esperado $E(\dddot{E}|_{n_s}(\chi_A))$ es igual a $E(\chi_A)$, que a su vez es igual a $\int \chi_A dP$, por definición $P(A)$. Resumiendo, el valor esperado de $P_s(A)$ es $P(A)$, al cual converge a medida que el tamaño de $s$ aumenta, si los valores $\mu({i})$ son acotados pero $\mu(\mathbb{N})$ es infinito, por la ley de los grandes números.
...
Sea $(\Omega, \mathcal{B})$ un espacio de medida cualquiera fijo. Sea $g$ una función de $\Omega$ en $\mathcal{B}$ que a cada elemento $x$ de $\Omega$ le asigna un evento $g(x)$, tal que $\mathcal{B}$ coincide con el álgebra generada por el rango de $f$. Es decir $\sigma(g(\Omega)) = \mathcal{B}$. Entonces $g$ se denomina función elemental.
...
Por ejemplo, sobre $(\mathbb{N}, \wp)$ se define la función elemental $g$, que a cada elemento le asigna su conjunto unitario: $g(i) = \{i\}$, de manera que efectivamente $\sigma(g(\mathbb{N})) = \wp$.
...
Otro ejemplo, sobre $(\mathbb{R}, \mathcal{B})$ se define la función elemental $g$, que a cada elemento le asigna su cola abierta a izquierda: $g(x) = (-\infty, x)$, de manera que efectivamente $\sigma(g(\mathbb{R})) = \mathcal{B}$.
...
Dados $(\Omega, \mathcal{B})$ un espacio de medida cualquiera fijo y una función elemental $g$, sí dos medidas de probabilidad $P_1$ y $P_2$ tienen la misma función de densidad general entonces son iguales entre sí.

Dados $(\Omega, \mathcal{B})$ un espacio de medida cualquiera fijo, una función elemental $g$ y una nube de puntos $s: (1:n, \wp, p) \rightarrow (\Omega, \mathcal{B})$. La medida de probabilidad empírica $P_s$ definida previamente, tiene una función de densidad general, que en el caso contínuo se denomina función de densidad acumulada empirica.
...
# Por corregir o borrar

## Subnubes

Sea $s: (1:n,\wp, p) \rightarrow (\Omega, \mathcal{B})$ una nube de puntos. Sea $n'$ menor o igual que $n$, y $t$ una función inyectiva de $1:n'$ en $1:n$, y $p'$ la probabilidad inducida, esto es: para todo $j$ en $1:n'$, $p'(\{j\}) = p(\{t(j)\})/p(t(1:n'))$. Entonces $s \circ t: (1:n',\wp, p') \rightarrow (\Omega, \mathcal{B})$ es una nube de puntos.

Dadas $s_1$ y $s_2$ nubes de puntos de $(1:n_1,\wp, p_1)$ y $(1:n_2,\wp, p_2)$ en $(\Omega, \mathcal{B})$ respectivamente, con $n_1$ menor o igual que $n_2$, si existe una función inyectiva $t$ de $1:n_1$ en $1:n_2$ tal que, para todo $j$ en $1:n_1$, $p_1(\{j\}) = p_2(\{t(j)\})/p_2(t(1:n_2))$, entonces se dice que $s_1$ es una subnube de $s_2$ y se nota $s_1 \leq s_2$

Dadas $s_1$ y $s_2$ nubes de puntos de $(1:n_1,\wp, p_1)$ y $(1:n_2,\wp, p_2)$ en $(\Omega, \mathcal{B})$ respectivamente, si $s_1 \leq s_2$ y $s_2 \leq s_1$ entonces $s_1 \equiv s_2$.

En tanto $\leq$ es relación de orden existen las operaciones binarias sup $\wedge$ e inf $\lor$.

## Sistemas

Dada una nube de puntos $s$ sobre $(\Omega, \mathcal{B})$, el conjunto de todas las subnubes de $s$ se denomina el sistema generado por $s$ y se nota $\mathfrak{w}_s$. 

Todo sistema es un retículo con la relación de orden $\leq$.

Un subsistema $\mathfrak{W} \subset \mathfrak{W}_s$ es un subconjunto propio de un sistema.

Un subsistema particional es aquel que cumple:

 - $Ran(s_1) \cap Ran(s_2) = \emptyset$ para todo par de nubes de puntos $s_1, s_2$ en $\mathfrak{W}$
 
 - $\bigwedge \mathfrak{W} = s$

Los subsistemas particionales han sido objeto de gran estudio en estadística por propiedades muy importantes como la descomposición de la media y de la varianza.

En particular, una variable de tipo categórico $x$ puede particionar una nube de puntos en subnubes. generada por una variable categórica^[formalizar variables categóricas]
