...
opts_chunk$set(echo = FALSE, include = FALSE, eval = TRUE, cache = TRUE, fig.width = 5.5, fig.height = 5)
...
Muestra %>% as.list %>% split(des) %>% lapply(data.frame, id_gen, stringsAsFactors = FALSE) %>% lapply(melt, "id_gen") %>% Map(data.frame, ., condi = unique(des)) %>%  do.call(rbind, .) -> datos
...
```{r, eval=FALSE}
  #K nubes
datos %>% mutate(gen_cond = paste(id_gen, condi, sep = "_")) %$% split(value, gen_cond) %>% lapply(to_nubepuntos) -> nubes

start.time <- Sys.time()
k_clouds(nubes, n_grupos = n.clusters, iter_max = 2) -> K_nubes_genes
end.time <- Sys.time()
...
muestra_agrupada %>% ggplot + aes(x = as.factor(grupo), y = value) + geom_boxplot() + theme_minimal() + labs(x = "Grupo", y = "")
