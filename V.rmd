---
title: "Aplicación: Datos Genómicos"
...
```{r}
totales <- muestra_nueva %$% split(value, cond_gen)
...
pvalues %>% as.vector %>% data.frame(expand.grid(1:nrow(pvalues),1:nrow(pvalues))) %>% mutate(valor = . < 0.00125) %>% ggplot + aes(x = Var1, y = Var2, fill = valor) + geom_raster() + theme_minimal(base_size = 6) + labs(x ="", y = "", fill = "Diferencias\nsignificativas")

function(nombre_gen){
unique(muestra_agrupada[muestra_agrupada$cond_gen %in% paste(c("cond_1", "cond_2"), nombre_gen, sep = "_"),c("cond_gen", "grupo")]) %>% "[["("grupo") %>% "["(pvalues, .)
} # aquíi voy
# nombre_gen <- "1-1-2.2.18.8"
