...
El rasterplot representa cada municipio con una línea que va de oscuro a claro, aquí tampoco es posible distinguir cada municipio, no obstante, permite observar la diversidad de los mismos, como se ve en la figura NBV.
...
Ambas visualizaciones presentan un panorama diverso en cuanto a los distintos desempeños y muy comprimido debido a la cantidad de municipios. Ambas visualizaciones han sido diseñadas con el fin de evidenciar la fuerte diferencia que hay entre un panorama inicial, con alteraciones casi aleatorias, y uno agregado. Dejando claro el anorama actual de los datos es factible pasar a la implementación propiamente dicha de los métodos presentados.
...
En las gráficas anteriores, EPN y NBV se observa la diversidad de los municipios en términos de resultados en la prueba Saber 11. El paso siguente consiste en aplicar los métodos presentados con el fin de agrupar los municipios según el desempeño de sus estudiantes. J-nubes produce como primer resultado un primer árbol, se trata de una estructura jerárquica de agrupación generada mediante el método de ward. Este árbol, en la figura GGB, constituye un valioso insumo para determinar el número de grupos en que se quiere dividir a los municipios.
...
Figura GGB: árbol de agrupación jerárquica generado por J-nubes.
...
Por medio del método J-nubes se construyen `r numero_grupos` grupos iniciales que se usan como punto de partida para K-nubes. De esta forma se consolidan `r numero_grupos` grupos cuyos nombres `r letters[1:numero_grupos]` corresponden con su nivel educativo, siendo `r letters[1]` el grupo de municipios con mejor desempeño en las pruebas Saber 11 para el año 2014 y `r letters[numero_grupos]` el grupo de municipios con puntajes totales más bajos. La tabla MBJ muestra las composiciones porcentuales de cada uno de los grupos. Se observa como los grupos más vastos son los que agrupan municipios de desempeños medios; mientras los grupos extremos, con muy altos o muy bajos rendimientos, son poco poblados.
...
```{r, eval=FALSE}
  #K nubes
 k_clouds(nubes, grupos_iniciales = grupos_municipios, iter_max = 10) -> K_nubes_municipios
...
ecdf_eval %>% apply(MARGIN = 1, FUN = sum) %>% aggregate(., by=list(K_nubes_municipios$Grupo), FUN = mean) %$% x %>% rank %>% setNames(letters[1:numero_grupos]) -> llave


letters[llave[K_nubes_municipios$Grupo]] -> grupos_municipios_2
...
El resultado es una categorización robusta que agrupa a los municipios no sólo a través de su promedio sino haciendo uso completo de los datos de cada uno. Las aproximaciones visuales anteriormente mostradas fueron diseñadas particular mente rara evidenciar esto. Resulta notorio el efecto del método al comparar el gráfico de caja global con su análogo agrupado en la figura PPP.
...
La labor de validación de los procedimientos se realiza vía Kruskall-Wallis. Se toma cada uno de los grupos de municipios y se aplica esta prueba; se espera que, si las condiciones distribucionales han sido capturadas por el método de agrupación, la prueba no encuentre diferencias estadísticamente significativas al interior de los grupos. No obstante, como se muestra en la figura TYY, esto no ocurre para todos los grupos.
